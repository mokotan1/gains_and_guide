...
# Groq 무료 티어 TPM 초과 시 1회 재시도용 (입력 토큰 축소)
_COMPACT_RAG_SNIPPET_CHARS = 280
_COMPACT_LONG_FIELD_MAX_CHARS = 3000
...
def _invoke_legacy_chat_resolving_tpm(
    client: Groq,
    get_prompts: Callable[[bool], tuple[str, str]],
) -> dict[str, Any]:
    ...
    try:
        ...
        sp2, uc2 = get_prompts(True)
        return _legacy_chat_completion(client, sp2, uc2)


def _chat_prompt_pair(
    body: ChatRequest, user_subject: str, compact: bool
) -> tuple[str, str]:
    rag_max = _COMPACT_RAG_SNIPPET_CHARS if compact else None
    field_cap = _COMPACT_LONG_FIELD_MAX_CHARS if compact else _long_text_field_cap()
    system_prompt = _build_chat_system_base(
        body.message, user_subject, rag_snippet_max=rag_max
    )
...
def _legacy_chat_completion(
    ...
    chat_completion = _groq_chat_completion_create(client, messages)
...
def _recommend_messages(
    body: RecommendRequest, user_subject: str, compact: bool
) -> list[dict[str, str]]:
    rag_max = _COMPACT_RAG_SNIPPET_CHARS if compact else None
    field_cap = _COMPACT_LONG_FIELD_MAX_CHARS if compact else _long_text_field_cap()
    ...
    system_prompt = _build_recommend_system_base(
        summary, user_subject, rag_snippet_max=rag_max
    )
    user_blob = (
        f"[주간 운동 분석 데이터]\n{summary}\n\n"
...
async def chat_with_coach(request: Request, body: ChatRequest) -> dict[str, Any]:
    ...
    if _use_legacy_chat() or not app_deps.coach_agent:
        try:
            raw = _invoke_legacy_chat_resolving_tpm(
                ...
                lambda compact: _chat_prompt_pair(body, user_subject, compact),
            )
            try:
    ...
    sp, uc = _chat_prompt_pair(body, user_subject, False)
    try:
        try:
            parsed = await _run_agent_with_timeout(sp, uc)
        except Exception as e:
            if _is_groq_tpm_rate_limit(e):
                logger.warning("coach agent: TPM/rate limit — compact prompt retry")
                sp, uc = _chat_prompt_pair(body, user_subject, True)
                parsed = await _run_agent_with_timeout(sp, uc)
            else:
                raise
        ...
        try:
            ...
            try:
                ...
                raw = _invoke_legacy_chat_resolving_tpm(
                    app_deps.groq_client,
                    lambda c: _chat_prompt_pair(body, user_subject, c),
                )
                try:
                    v = _coerce_chat_response(raw)
                except ValidationError:
...
async def recommend_routine(request: Request, body: RecommendRequest) -> dict[str, Any]:
    ...
    messages = _recommend_messages(body, user_subject, False)

    reply: Optional[str] = None
    try:

        def _groq_call() -> Any:
            return _groq_chat_completion_create(app_deps.groq_client, messages)
        ...
        try:
            ...
            messages = _recommend_messages(body, user_subject, True)

            def _groq_call_compact() -> Any:
                return _groq_chat_completion_create(app_deps.groq_client, messages)

            chat_completion = await asyncio.wait_for(
                asyncio.to_thread(_groq_call_compact),
                timeout=_coach_timeout_sec(),
            )
        reply = chat_completion.choices[0].message.content
        if not reply:
