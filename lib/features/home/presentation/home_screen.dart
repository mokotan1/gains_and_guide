...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void _processAiRecommendation(List<Exercise> currentExercises) async {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('AI 코치가 분석 중입니다...', style: TextStyle(fontWeight: FontWeight.bold))
              ],
            ),
          ),
        ),
    ...
    String summary = currentExercises
        .map((e) => "${e.name}: ${e.weight}kg x ${e.sets}세트 (RPE: ${e.setRpe.join(',')})")
        .join('\n');
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'user_id': 'master_user',
          'message': '$pInfo 오늘 운동 기록을 바탕으로 분석 및 가이드를 제공해줘.',
          'context': summary,
        }),
      );
      ...
      if (response.statusCode == 200) {
        ...
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: SingleChildScrollView(child: Text(data['response'])),
