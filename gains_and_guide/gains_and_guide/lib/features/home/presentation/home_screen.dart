...
import '../../workout/application/workout_service.dart';
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Future<String> _generateWorkoutCsv(List<Exercise> currentExercises) async {
    ...
    final history = await ref.read(workoutServiceProvider).getAllHistory();
  ...
  void _processAiRecommendation(List<Exercise> currentExercises) async {
    ...
    try {
      ...
      // 2. 프로필 및 데이터 준비
      final profileMap = await ref.read(bodyProfileRepositoryProvider).get();
      String pInfo = profileMap != null ? "사용자 체중: ${profileMap.weight}kg. " : "";
  ...
  void _showAddExerciseDialog() async {
    ...
    for (var row in catalogList) {
      final name = row.name;
      final muscles = row.primaryMuscles.toLowerCase();
