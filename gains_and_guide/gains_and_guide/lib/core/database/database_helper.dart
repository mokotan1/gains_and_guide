...
/// DB 연결·스키마 전담. 도메인 타입 없이 Map/원시 타입만 반환 (DDD: 인프라 경계)
class DatabaseHelper {
  ...
  // 운동 카탈로그 검색 (도메인 무의존: Map 반환)
  Future<List<Map<String, dynamic>>> searchCatalogExercisesRaw(String keyword) async {
    final db = await instance.database;
  ...
  Future<List<Map<String, dynamic>>> getAllCatalogRaw() async {
    final db = await instance.database;
    return await db.query('exercise_catalog');
  }
