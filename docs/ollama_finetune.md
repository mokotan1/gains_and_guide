...
(실제 CLI는 LLaMA-Factory 버전에 따라 `llamafactory-cli` 또는 `python -m llamafactory.cli train ...` 일 수 있음 — 공식 README 확인)

**베이스 모델**

- 게이트 없이 시작하기 쉬운 예: `Qwen/Qwen2.5-3B-Instruct` + `template: qwen`
- Llama: `meta-llama/Llama-3.2-3B-Instruct` + `template: llama3` — Hugging Face **모델 접근 승인 + 승인 계정 토큰** 필요 (미승인 시 `GatedRepoError` / 403)

**학습 환경에서 자주 나는 이슈**

- **T4 등 구형 GPU**: `bf16: true`가 실패할 수 있어 `fp16: true`, `bf16: false`, `cutoff_len` 축소를 검토
- **VRAM 부족**: `per_device_train_batch_size`, `cutoff_len`, `lora_rank` 낮추기
- 로그에 **데이터 경로·템플릿 이름·OOM** 등 원인이 직접 나오므로 stderr를 먼저 확인
